/** Attach the real T3CP source documents to the OSW26BZ04-DP013 solicitation through the product's
 *  admin RFP-upload route, which emits finder:rfp.uploaded → OnRfpUploaded workflow → shredder. */
...
const FILES: Array<[string,string,string]> = [
  ...
  ['bfbbeae0-topic_OSW26BZ04DP013_T3CP_Patent_Holiday_SBIR_Open_Topic_Call.DOCX','topic_OSW26BZ04DP013_T3CP_Patent_Holiday_SBIR_Open_Topic_Call.docx','application/vnd.openxmlformats-officedocument.wordprocessingml.document'],
];
...
fd.append('documentTypes', JSON.stringify(['topic','instructions','rfp','topic']));
fd.append('isPrimary','true');
for (const [src,name,type] of FILES) {
  fd.append('files', new File([new Uint8Array(readFileSync(`${U}/${src}`))], name, { type }));
}
const r=await page.request.post(`${BASE}/api/admin/rfp-upload`,{multipart:fd,timeout:300000});
console.log('upload HTTP', r.status(), (await r.text()).slice(0,400));
await b.close();
