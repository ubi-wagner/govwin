 ...
 *
 * Migration 216 scopes every delivery table by tenant, and `verify-project-isolation.mjs` proves
 ...
 *
 *   · the query for a plain employee JOINS `project_assignments` — without the join it returns
...
import {
  projectScope, canAccessProject, listProjectsForActor, canAssign,
...
describe('projectScope', () => {
  it('tenant_admin and above see every project at their tenant', () => {
    for (const role of ['tenant_admin', 'rfp_admin', 'master_admin']) {
      expect(projectScope({ role, userId: USER }), role).toEqual({ kind: 'all' });
    }
  });

  it('a tenant_user is scoped to what they are assigned', () => {
    expect(projectScope({ role: 'tenant_user', userId: USER })).toEqual({ kind: 'assigned', userId: USER });
  });
  ...
  it('partner_user is refused outright — membership or not', () => {
    ...
    // wrote down.
    expect(projectScope({ role: 'partner_user', userId: USER })).toEqual({ kind: 'none' });
  });
  ...
  it('a bare partner_admin has no delivery reach of its own', () => {
    ...
    // right reading.
    expect(projectScope({ role: 'partner_admin', userId: USER })).toEqual({ kind: 'none' });
  });

  it('an unrecognised role is refused', () => {
    expect(projectScope({ role: 'auditor', userId: USER })).toEqual({ kind: 'none' });
    expect(projectScope({ role: '', userId: USER })).toEqual({ kind: 'none' });
  });
});
...
describe('listProjectsForActor — the query itself', () => {
  it('an employee query JOINS project_assignments', async () => {
    sqlMock.rows.current = [];
    ...
    expect(q, 'without this join the query returns EVERY project at the tenant, and every RLS '
      + 'assertion still passes').toMatch(/JOIN project_assignments/i);
...
describe('canAccessProject', () => {
  it('an assigned employee reaching their own project uses the assignment join', async () => {
    ...
    expect(lastQuery()).toMatch(/JOIN project_assignments/i);
  });
  ...
  it('a tenant_admin reaches it without an assignment row', async () => {
    ...
    expect(allowed).toBe(true);
    expect(lastQuery()).not.toMatch(/project_assignments/i);
  });
