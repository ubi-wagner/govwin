 ...
 *
 *   lib/projects/milestones.ts   the milestone variance in the `project.milestone.met` payload —
 *                                NaN, and `JSON.stringify(NaN)` is `null`, so every met milestone
 *                                recorded "no baseline" instead of "nine days late", permanently
 *   lib/projects/baseline.ts     the ALREADY_BASELINED message, shown to a person, reading
...
describe('the delivery module does not slice the string form of a date', () => {
  const ROOTS = ['lib/delivery', 'components/delivery', 'app/portal/[tenantSlug]/projects'];
  ...
  it('has no `String(…).slice(0, 10)` in delivery code', async () => {
    ...
    expect(offenders, 'use isoDate()/daysBetween() from lib/projects/dates.ts').toEqual([]);
  });
});
