 ...
 *
 * Migration 216 enforces immutability in a TRIGGER, and `verify-delivery-isolation.mjs` proves that
...
vi.mock('@/lib/delivery/access', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@/lib/delivery/access')>()),
  canAccessProject: vi.fn(async () => true),
}));

import { setBaseline, rebaseline } from '@/lib/delivery/baseline';
...
describe('setBaseline', () => {
  ...
  it('REFUSES a project that is already baselined — legibly, before the trigger has to', async () => {
    ...
    expect(allQueries(), 'nothing may be written on a refusal').not.toMatch(/UPDATE delivery_wbs_nodes/i);
  });
  ...
  it('REFUSES when an anchor document is missing, and names which one', async () => {
    ...
    expect(allQueries()).not.toMatch(/UPDATE delivery_wbs_nodes/i);
  });
  ...
  it('claims the project by compare-and-swap, so two requests cannot both win', async () => {
    queueProject(null, bothDocs);
    await setBaseline(ADMIN, PROJECT);
    expect(allQueries()).toMatch(/UPDATE delivery_projects .* baselined_at IS NULL/i);
  });
...
describe('rebaseline', () => {
  ...
  it('leaves a MET milestone where it is', async () => {
    ...
    const writes = db.state.queries.filter((q) => /delivery_milestones/i.test(q)).join(' ');
