 ...
 *
 * ── WHAT HAPPENED ────────────────────────────────────────────────────────────────────────────
 * `projectGate` called `enterTenant()` from inside itself. `AsyncLocalStorage.enterWith` sets the
 ...
 * `withProject` runs the handler inside `runInTenant` (`store.run()`), the primitive that actually
...
const API = join(ROOT, 'app/api/portal/[tenantSlug]/projects');
...
describe('delivery API routes are scoped to the tenant context', () => {
  ...
  it('no route imports the raw gate — every handler goes through withProject', () => {
    const offenders = files
      .map((f) => ({ rel: relative(ROOT, f), src: readFileSync(f, 'utf8') }))
      .filter(({ src }) => /\bprojectGate\b/.test(src))
      .map(({ rel }) => rel);
    expect(offenders, 'use withProject(tenantSlug, async (gate) => …) from lib/projects/gate')
      .toEqual([]);
  });
  ...
  it('every handler body is wrapped', () => {
    ...
    for (const f of files) {
      ...
      const wrapped = (src.match(/\bwithProject\(/g) ?? []).length;
  ...
  it('the gate itself does not enterWith — that is what silently unscoped every route', () => {
    const gate = readFileSync(join(ROOT, 'lib/projects/gate.ts'), 'utf8');
