/**
 * Does the delivery spine isolate, and does the baseline actually refuse to move?
 *
 ...
 * express) is a vitest file: `__tests__/delivery-assignment-boundary.test.ts`.
 *
 ...
 *
 *   source scripts/sandbox-env.sh && node frontend/scripts/verify-delivery-isolation.mjs
 *
...
async function main() {
  ...
  const [{ n: reserved }] = await owner`
    SELECT count(*)::int AS n FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name LIKE 'delivery\\_%' AND column_name = 'current_date'`;
  if (reserved) no('a delivery table has a column literally named `current_date` — a reserved keyword');
  else ok('no delivery column is named with a reserved keyword');
  ...
  const [pA] = await owner`
    INSERT INTO delivery_projects (tenant_id, name) VALUES (${A.id}, ${`probe-A-${stamp}`}) RETURNING id`;
  const [pB] = await owner`
    INSERT INTO delivery_projects (tenant_id, name) VALUES (${B.id}, ${`probe-B-${stamp}`}) RETURNING id`;
  created.projects.push(pA.id, pB.id);

  await owner`
    INSERT INTO delivery_wbs_nodes (tenant_id, project_id, code, title, planned_cost)
    VALUES (${A.id}, ${pA.id}, '1.1', 'Probe task', 1000)`;
  await owner`
    INSERT INTO delivery_milestones (tenant_id, project_id, title) VALUES (${A.id}, ${pA.id}, 'Probe milestone')`;
  ...
  const seen = await app.begin(async (tx) => {
    ...
    const [own] = await tx`SELECT count(*)::int AS n FROM delivery_projects WHERE id = ${pB.id}`;
    const [foreign] = await tx`SELECT count(*)::int AS n FROM delivery_projects WHERE id = ${pA.id}`;
    const [foreignWbs] = await tx`SELECT count(*)::int AS n FROM delivery_wbs_nodes WHERE project_id = ${pA.id}`;
  ...
  const writeCode = await refusedWith(() => app.begin(async (tx) => {
    await tx`SELECT set_config('app.tenant_id', ${B.id}, true)`;
    await tx`INSERT INTO delivery_wbs_nodes (tenant_id, project_id, code, title)
             VALUES (${A.id}, ${pA.id}, '9.9', 'injected')`;
  }));
  if (writeCode !== '42501') {
    no(`a cross-tenant INSERT ${writeCode === null ? 'SUCCEEDED' : `raised ${writeCode}`} — expected 42501`);
    if (writeCode === null) await owner`DELETE FROM delivery_wbs_nodes WHERE code = '9.9' AND project_id = ${pA.id}`;
  ...
  const [node] = await owner`SELECT id FROM delivery_wbs_nodes WHERE project_id = ${pA.id} LIMIT 1`;

  const setCode = await refusedWith(() => owner`
    UPDATE delivery_wbs_nodes SET baseline_start = '2026-01-01', baseline_cost = 1000 WHERE id = ${node.id}`);
  ...
  const rewriteSameCode = await refusedWith(() => owner`
    UPDATE delivery_wbs_nodes SET baseline_start = '2026-01-01', title = 'Probe task renamed' WHERE id = ${node.id}`);
  ...
  const moveCode = await refusedWith(() => owner`
    UPDATE delivery_wbs_nodes SET baseline_start = '2026-06-01' WHERE id = ${node.id}`);
  if (moveCode !== '23001') {
  ...
  const clearCode = await refusedWith(() => owner`
    UPDATE delivery_wbs_nodes SET baseline_cost = NULL WHERE id = ${node.id}`);
  ...
  const planCode = await refusedWith(() => owner`
    UPDATE delivery_wbs_nodes SET planned_start = '2026-07-01', planned_cost = 2500, actual_cost = 400
     WHERE id = ${node.id}`);
  ...
  const [ms] = await owner`SELECT id FROM delivery_milestones WHERE project_id = ${pA.id} LIMIT 1`;
  await owner`UPDATE delivery_milestones SET baseline_date = '2026-03-01', forecast_date = '2026-03-01' WHERE id = ${ms.id}`;
  const msMove = await refusedWith(() => owner`UPDATE delivery_milestones SET baseline_date = '2026-04-01' WHERE id = ${ms.id}`);
  const msForecast = await refusedWith(() => owner`UPDATE delivery_milestones SET forecast_date = '2026-04-15' WHERE id = ${ms.id}`);
...
async function finish() {
  if (created.projects.length) {
    ...
    try { await owner`DELETE FROM delivery_projects WHERE id IN ${owner(created.projects)}`; }
    catch (err) { console.error('  cleanup failed:', err?.message ?? err); }
    console.log();
    console.log(`  MUTATED ${created.projects.length} delivery_projects row(s) and their cascade — `
