...
import { COST_SPECS, TECH_VOLUMES, buildFilledCost, readTech, techDoc, HERE } from './_shared.mts';
...
async function main() {
  ...
  for (const t of TECH_VOLUMES) {
    ...
    const docx = await exportToDocx(doc, V); writeFileSync(join(OUT, `NILOC-${t.tag}-technical.docx`), docx);
    let note = '';
    try { const pdf = await exportToPdf(doc, V); writeFileSync(join(OUT, `NILOC-${t.tag}-technical.pdf`), pdf); note = `pdf ${(pdf.length / 1024).toFixed(0)}KB`; }
    catch (e) { note = 'pdf skipped (' + String(e).slice(0, 48) + ')'; }
    console.log(`TECH ${t.tag.padEnd(10)} docx ${(docx.length / 1024).toFixed(0)}KB · ${note}`);
  }
  for (const spec of COST_SPECS) {
    ...
    console.log(`COST ${spec.tag.padEnd(10)} xlsx ${(xlsx.length / 1024).toFixed(0)}KB · ${note}`);
  }
