...
L.push(`| API routes (write verbs) | ${apis.filter((r) => (r.methods ?? []).some((m) => m !== 'GET')).length} | verify-write-contract (envelope, all of them) · verify-db-crud (effects, a chosen subset) | 0 ungraded for envelope; effects not enumerated |`);
...
L.push('');
L.push('**The write verbs are graded for SHAPE, not for EFFECT, and the distinction is the gap.** '
  + apis.filter((r) => (r.methods ?? []).some((m) => m !== 'GET')).length + ' routes expose a');
L.push('POST/PATCH/PUT/DELETE. `verify-api-contract` cannot touch them — it is GET-only by');
L.push('construction, because calling every write verb would mutate the box it is measuring — so');
L.push('`verify-write-contract` exists to walk them, binding every `[param]` to a uuid owning nothing');
L.push('and asserting the one property that needs no successful write: a client error answers 4xx');
L.push('with both `error` and `code`, never 500. That is every write verb covered for its REFUSAL');
L.push('path and none of them covered for what a SUCCESSFUL call does; `verify-db-crud` proves that');
L.push('half for a hand-picked set of invariants rather than by enumeration. Written down here so the');
L.push('next reader does not read a walked API off five green lenses.');
L.push('');
L.push('The same split applies one level down, and is the wider of the two: an `{error, code}` at the');
L.push('right status says the caller was told. Whether anything but the caller was told is a separate');
L.push('question, measured by `audit-refusal-observability` and `drive-failure-observability`.');
L.push('');
