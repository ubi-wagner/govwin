...
L.push('> The four lenses each enumerate their own scope, so anything belonging to neither walk has');
L.push('> never appeared in a coverage number. A surface a lens has no expectation for is **uncovered,');
L.push('> not passing.** The COVERAGE column below is the join that makes that visible.');
L.push('>');
