 ...
 *
 *   source scripts/sandbox-env.sh && npx tsx frontend/scripts/verify-delivery-rollup.mjs
 *
...
async function main() {
  const { rollup } = await import('../lib/delivery/rollup.ts');
  ...
  const [p] = await owner`
    INSERT INTO delivery_projects (tenant_id, name) VALUES (${tenant.id}, ${`rollup-probe-${process.pid}`})
    RETURNING id`;
  projectId = p.id;

  const [clinA] = await owner`
    INSERT INTO delivery_clins (tenant_id, project_id, clin_number, title, sort_index)
  ...
  const [ms] = await owner`
    INSERT INTO delivery_milestones (tenant_id, project_id, clin_id, title, sort_index)
  ...
  // ── measure ────────────────────────────────────────────────────────────────────────────────
  //
  // ENTER THE TENANT CONTEXT FIRST, exactly as `deliveryGate` does on a real request. Without it
  ...
  //
  // That silent-empty is the whole reason `verify-delivery-isolation.mjs` asserts own-rows-visible
...
async function cleanup() {
  if (projectId) {
    try { await owner`DELETE FROM delivery_projects WHERE id = ${projectId}`; }
    catch (err) { console.error('  cleanup failed:', err?.message ?? err); }
    console.log(`\n  MUTATED 1 delivery_projects row and its cascade — fixture-only, now removed.`);
  }
  await owner.end();
}
