    ...
    try {
      ...
      if (resp.ok) {
        ...
        setRecords(rows.map((row) => ({
          ...
          // @/lib/db camelCases result columns — the API passes them through, so read camelCase.
          template_type: row.templateType as string,
          agency: (row.agency as string) ?? null,
          program_type: (row.programType as string) ?? null,
          template_key: (row.storageKey as string) ?? (row.id as string),
          node_count: (row.nodeCount as number) ?? 0,
          format: ((row.canvasPreset as Record<string, unknown>)?.format as string) ?? 'letter',
          page_limit: ((row.canvasPreset as Record<string, unknown>)?.max_pages as number) ?? null,
          slide_limit: ((row.canvasPreset as Record<string, unknown>)?.max_slides as number) ?? null,
          is_system: (row.isSystem as boolean) ?? false,
          canvas_doc: null,
        })));
      }
    } catch {
