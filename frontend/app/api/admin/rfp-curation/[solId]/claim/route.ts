/**
 * POST /api/admin/rfp-curation/[solId]/claim
 *
 * Claims an unclaimed solicitation for curation. Delegates to the
 * `solicitation.claim` tool via the registry for atomic state
 * transition, audit logging, and event emission.
 *
 * Returns: { data: { solicitationId, status, claimedBy, claimedAt } }
 */
...
export async function POST(
  ...
  try {
    ...
    const toolCtx: ToolContext = {
      ...
      requestId,
      log: log.child({ tool: 'solicitation.claim', requestId }),
    };

    // ── Invoke tool ─────────────────────────────────────────────────
    const result = await invoke('solicitation.claim', { solicitationId: solId }, toolCtx);
    ...
    console.error('[rfp-curation] claim failed:', error);
    return NextResponse.json(
      { error: 'Failed to claim solicitation', code: 'INTERNAL_ERROR' },
      { status: 500 },
    );
  }
}
