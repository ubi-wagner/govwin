...
import { listProjectsForActor, projectScope, canAssign } from '@/lib/projects/access';

export const dynamic = 'force-dynamic';

/**
 * Projects — the contracts this tenant is executing.
 *
 ...
 * boundary test (`__tests__/projects-assignment-boundary.test.ts`).
 *
 * A `partner_user` never reaches here at all: `projectScope` refuses the role, and the nav does not
 * render the link. Projects v1 has no collaborator surface, which is what removes cross-tenant from
...
export default async function ProjectsPage({
  ...
  const scope = projectScope({ role, userId: su.id });
      ...
      {projects.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-8 text-center">
          <p className="text-sm font-medium text-gray-900">
            {admin ? 'No project workspaces yet' : 'You are not assigned to a project workspace'}
          </p>
          <p className="mx-auto mt-2 max-w-lg text-sm text-gray-600">
            {admin
              ? 'A workspace opens when a proposal is recorded as awarded — you will get a “Set up '
                + 'project workspace” task. It starts with two uploads: the executed contract and '
              ...
              {projects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link
                      href={`/portal/${tenantSlug}/projects/${p.id}`}
