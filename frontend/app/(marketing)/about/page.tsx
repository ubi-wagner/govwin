    ...
    <>
      ...
      <Section className="bg-surface-50">
        <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { val: 'AI', label: 'Scoring Engine', desc: 'Multi-factor relevance scoring using NAICS, keywords, set-asides, and agency history' },
              { val: '24/7', label: 'Monitoring', desc: 'Continuous scanning of SAM.gov and federal procurement sources' },
              { val: 'SaaS', label: 'Multi-Tenant', desc: 'Secure, isolated workspaces for every client organization' },
              { val: 'Fast', label: 'Setup', desc: 'Enter your profile, get scored opportunities in minutes — not weeks' },
            ].map(item => (
              <div key={item.val} className="group rounded-2xl bg-white p-5 shadow-card border border-gray-200/60 transition-all duration-300 hover:shadow-card-hover hover:-translate-y-0.5">
                <p className="text-3xl font-extrabold bg-gradient-to-r from-brand-600 to-brand-500 bg-clip-text text-transparent">{item.val}</p>
      ...
      <Section className="bg-white">
        ...
        <div className="mt-14 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-gray-200 bg-gray-200/60 sm:grid-cols-2 lg:grid-cols-3">
          {[
            { title: 'Information Overload', desc: 'Thousands of new opportunities posted daily — most irrelevant to your business.', icon: '01' },
            { title: 'Missed Deadlines', desc: 'Critical response windows close before you even discover the opportunity.', icon: '02' },
            { title: 'Manual Searching', desc: 'Hours spent on SAM.gov with clunky filters that return noisy results.', icon: '03' },
            { title: 'No Prioritization', desc: 'Every opportunity looks the same — no way to focus on what you can actually win.', icon: '04' },
            { title: 'Fragmented Tools', desc: 'Spreadsheets, email chains, and browser bookmarks instead of a real pipeline.', icon: '05' },
            { title: 'Wasted Proposals', desc: 'Time spent pursuing opportunities that were never a good fit to begin with.', icon: '06' },
          ].map(item => (
            <div key={item.title} className="relative bg-white p-6 group hover:bg-brand-50/30 transition-colors">
              <span className="absolute right-4 top-4 text-3xl font-black text-gray-100 group-hover:text-brand-100 transition-colors">{item.icon}</span>
