...
export async function createWbsNode(
  ...
  try {
    ...
    for (const [label, id, table] of [
      ['parentId', input.parentId, 'delivery_wbs_nodes'],
      ['clinId', input.clinId, 'delivery_clins'],
    ] as const) {
      if (!id) continue;
      const rows = table === 'delivery_wbs_nodes'
        ? await sql<{ id: string }[]>`
            SELECT id FROM delivery_wbs_nodes
             WHERE id = ${id}::uuid AND project_id = ${projectId}::uuid LIMIT 1`
        : await sql<{ id: string }[]>`
            SELECT id FROM delivery_clins
             WHERE id = ${id}::uuid AND project_id = ${projectId}::uuid LIMIT 1`;
      if (rows.length === 0) {
    ...
    await auditLog({
      tenantId: actor.tenantId, userId: actor.userId, action: 'delivery.wbs_node_created',
      entityType: 'delivery_wbs_node', entityId: row.id, metadata: { projectId, code, title },
    });
    return { ok: true, data: row };
  } catch (err) {
    console.error('[delivery/wbs] createWbsNode failed:', err);
