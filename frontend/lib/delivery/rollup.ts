 ...
 *
 * Exported for `__tests__/delivery-rollup-measures.test.ts`. The live drive
 * (`scripts/verify-delivery-rollup.mjs`) proves the SQL, but it needs a database and CI runs
...
export async function rollup(tenantId: string, projectId: string): Promise<ProjectRollup> {
  try {
    ...
    const clinRows = await sql<Array<{
      ...
      SELECT id, clin_number, title, funded_amount FROM delivery_clins
    ...
    console.error('[delivery/rollup] rollup failed:', err);
