/**
 * Who may see a delivery project — the layer RLS cannot express.
 *
 * ── WHY THIS MODULE EXISTS AT ALL ────────────────────────────────────────────────────────────
 * Row-level security scopes by TENANT, because the per-request context carries exactly one value:
 * `app.tenant_id`. Delivery needs a second, narrower scope — *which employees of that tenant*
 ...
 * (`__tests__/delivery-assignment-boundary.test.ts`) rather than relying on migration 216's policy.
 *
 * ── THE THREE WAYS IN ────────────────────────────────────────────────────────────────────────
 ...
 *      ceremony.
 *   2. A `tenant_user` with a `delivery_assignments` row for that project. Assignment is the whole
 ...
 * spine, where collaboration is the point. Delivery v1 has no collaborator surface at all, and that
...
export type DeliveryScope =
  | { kind: 'none' }                      // no delivery access at this tenant, for any project
 ...
 * question: given that they belong here, how much of delivery do they see?
 */
export function deliveryScope(actor: { role: string; userId: string }): DeliveryScope {
  ...
  // bare partner_admin role has no delivery reach of its own — which is the fail-closed reading.
...
export async function canAccessProject(
  ...
  const scope = deliveryScope(actor);
  if (scope.kind === 'none') return false;

  try {
    if (scope.kind === 'all') {
      const rows = await sql<{ id: string }[]>`
        SELECT id FROM delivery_projects
    ...
    console.error('[delivery/access] canAccessProject failed:', err);
    return false;
  }
}
 ...
 */
export async function listProjectsForActor(actor: DeliveryActor): Promise<DeliveryProjectRow[]> {
  const scope = deliveryScope(actor);
  if (scope.kind === 'none') return [];

  try {
    if (scope.kind === 'all') {
      return await sql<DeliveryProjectRow[]>`
    ...
    console.error('[delivery/access] listProjectsForActor failed:', err);
    return [];
  }
}
...
export async function listAssignees(tenantId: string, projectId: string): Promise<Array<{
  ...
  try {
    ...
    console.error('[delivery/access] listAssignees failed:', err);
    return [];
  }
}
