/**
 * Delivery projects and their anchor documents.
 *
 * ── THE ANCHOR ───────────────────────────────────────────────────────────────────────────────
 * A delivery workspace hangs off two UPLOADED files: the executed contract and the as-submitted
 ...
 * uploaded PDF traces to what was actually signed. `delivery_projects.contract_id` is a navigation
...
export async function createProject(
...
): Promise<Ok<DeliveryProject> | Fail> {
  if (!canAssign(actor.role)) {
    return { ok: false, status: 403, error: 'Only a tenant admin can open a delivery workspace', code: 'FORBIDDEN' };
  }
  ...
  try {
    // FK-BEFORE-WRITE. `delivery_projects.contract_id` has a real FK, so a bad id would throw AFTER
    ...
    const [row] = await sql<DeliveryProject[]>`
      INSERT INTO delivery_projects (tenant_id, contract_id, name, created_by)
    ...
    await auditLog({
      tenantId: actor.tenantId, userId: actor.userId, action: 'delivery.project_created',
      entityType: 'delivery_project', entityId: row.id, metadata: { name },
    });

    return { ok: true, data: row };
  } catch (err) {
    console.error('[delivery/projects] createProject failed:', err);
    return { ok: false, status: 500, error: 'Failed to create the delivery workspace', code: 'DB_ERROR' };
  }
}

export async function getProject(actor: DeliveryActor, projectId: string): Promise<DeliveryProject | null> {
  if (!(await canAccessProject(actor, projectId))) return null;
  try {
    const rows = await sql<DeliveryProject[]>`
    ...
    console.error('[delivery/projects] getProject failed:', err);
    return null;
  }
}
  ...
  try {
    return await sql<SourceDocument[]>`
      SELECT id, kind, storage_key, filename, content_type, byte_size, uploaded_by, uploaded_at
        FROM delivery_source_documents
       WHERE project_id = ${projectId}::uuid AND tenant_id = ${tenantId}::uuid
       ORDER BY uploaded_at`;
  } catch (err) {
    console.error('[delivery/projects] listSourceDocuments failed:', err);
    return [];
  }
}
...
export async function addSourceDocument(
  ...
  try {
    ...
    const key = customerDeliveryPath(tenant.slug, projectId, `source/${docId}.${ext}`);
    ...
    await auditLog({
      tenantId: actor.tenantId, userId: actor.userId, action: 'delivery.source_document_uploaded',
      entityType: 'delivery_project', entityId: projectId, metadata: { kind: input.kind, filename },
    });

    return { ok: true, data: row };
  } catch (err) {
    console.error('[delivery/projects] addSourceDocument failed:', err);
