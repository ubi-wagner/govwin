 ...
 * then mark done), or a form (fill the spec'd fields). Pure selection + spec
 * parsing so the rendering component stays declarative and this stays testable.
 */
export type CompleterKind = 'review' | 'upload' | 'form';
...
/** Which completer to render. Unknown/missing kind → the safe default 'review'. */
export function taskCompleterKind(params: Record<string, unknown> | null | undefined): CompleterKind {
  const kind = params && typeof params === 'object' ? (params as { kind?: unknown }).kind : undefined;
  return kind === 'upload' || kind === 'form' ? kind : 'review';
}
