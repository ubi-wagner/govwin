/**
 * CLINs — the contract line items a project is measured against.
 *
...
export async function listClins(tenantId: string, projectId: string): Promise<Clin[]> {
  try {
    ...
    console.error('[projects/clins] listClins failed:', err);
    return [];
  }
}
...
export async function createClin(
  ...
  try {
    ...
    console.error('[projects/clins] createClin failed:', err);
...
async function recordCitations(
  ...
  for (const [field, cite] of Object.entries(citations)) {
    if (!CITABLE_FIELDS.has(field)) {
      console.error(`[projects/clins] '${field}' is not a citable CLIN field — ignored`);
      continue;
    }
    await recordProvenance({
      tenantId: actor.tenantId,
      projectId,
      targetTable: 'project_clins',
      targetId: clinId,
      field,
      method: cite.method,
