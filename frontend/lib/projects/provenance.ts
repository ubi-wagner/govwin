/**
 * Where a project value came from — the ingest-provenance doctrine, one domain over.
 *
...
export async function recordProvenance(
  ...
  if (cites && !params.sourceDocId) {
    console.error(`[projects/provenance] method '${params.method}' requires a source document — `
    ...
    const rows = await sql<{ id: string }[]>`
      ...
      ON CONFLICT (target_table, target_id, field) DO UPDATE
        ...
        WHERE ${params.supersedes === true}::boolean
              ...
              < array_position(
                ARRAY['hitl','verified','override','pattern_match','ai','default'],
                project_provenance.method)
      RETURNING id`;
    return rows.length > 0;
  } catch (err) {
    console.error('[projects/provenance] recordProvenance failed:', err);
    return false;
  }
}
  ...
  try {
    const rows = await sql<FieldProvenance[]>`
      SELECT p.field, p.method, p.source_doc_id, p.page, p.excerpt, p.char_offset,
             d.filename
        FROM project_provenance p
        LEFT JOIN project_source_documents d ON d.id = p.source_doc_id
       WHERE p.tenant_id = ${tenantId}::uuid
         AND p.target_table = ${targetTable}
         AND p.target_id = ${targetId}::uuid`;
    return Object.fromEntries(rows.map((r) => [r.field, r]));
  } catch (err) {
    console.error('[projects/provenance] provenanceFor failed:', err);
    return {};
  }
}
