/**
 * The gate every project route runs first.
 *
 ...
 * rather than wrapping" — and then called `enterTenant()` from inside `projectGate` anyway. The
 ...
 * project route therefore executed with `app.tenant_id` UNSET, RLS matched nothing, and:
 *
 ...
 *
 * So the gate WRAPS. `withProject` runs the handler inside `runInTenant`, which uses `store.run()`
 * — the primitive that actually scopes a callback — and there is no way to hold the actor without
 * being inside the context. `__tests__/projects-gate-scoping.test.ts` fails if a route imports the
 * raw gate.
 *
 * ── WHAT THIS DELIBERATELY DOES NOT DO ───────────────────────────────────────────────────────
 * It does not check the ASSIGNMENT. That is a per-project question, RLS cannot express it, and it
 * lives in `lib/projects/access.ts` where it has its own boundary test. Folding it in here would
...
import { projectScope, type ProjectActor } from './access';

export type GateResult =
  | { error: NextResponse }
  | { actor: ProjectActor & { role: Role }; tenantSlug: string; email: string | null };

export async function projectGate(tenantSlug: string): Promise<GateResult> {
  ...
  // A role with no project reach at all is refused HERE rather than returning an empty list from
  ...
  if (projectScope({ role, userId: u.id }).kind === 'none') {
    return {
      error: NextResponse.json(
        { error: 'Projects are not available to this role', code: 'FORBIDDEN' },
        { status: 403 },
      ),
    };
  }

  // NOT `enterTenant` here — see the header. `withProject` scopes the handler instead.
  return { actor: { userId: u.id, role, tenantId }, tenantSlug, email: u.email ?? null };
}

/**
 * The only correct way into a project route.
 *
...
export async function withProject(
  ...
  const gate = await projectGate(tenantSlug);
