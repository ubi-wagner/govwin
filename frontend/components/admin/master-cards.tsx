          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((c) => {
                ...
                // data-opportunity-id makes a row ADDRESSABLE. Two solicitations can carry the same
                // title — two releases of one BAA legitimately do — so a harness locating a row by
                // its title text takes `.first()` and silently reads a different opportunity than
                // the one it just changed. drive-admin-demand did exactly that and reported the
                // demand count as never reaching the admin: a convincing product defect that was
                // not one.
                return (
                  <tr key={c.opportunityId} data-opportunity-id={c.opportunityId}
