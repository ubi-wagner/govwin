...
export default function LibraryDashboard({
  ...
  return (
    <div>
      {/* Atomize nudge — draft atoms from any upload await classify/tag/accept in the rail */}
      {draftCount > 0 && (
        <a
          href={`/portal/${tenantSlug}/library/review`}
          className="flex items-center justify-between gap-3 mb-6 px-4 py-3 rounded-xl border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 transition-colors"
        >
          <span className="flex items-center gap-2 text-sm text-indigo-800">
            ...
            <span>
              <span className="font-semibold">{draftCount} atom{draftCount !== 1 ? 's' : ''}</span> ready to classify, tag &amp; accept
            </span>
          </span>
          <span className="text-sm font-medium text-indigo-700 whitespace-nowrap">Atomize →</span>
        </a>
      )}
