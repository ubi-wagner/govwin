...
The remaining unsurfaced routes are dispositioned below (six when this was written; five as re-measured 2026-09-08). **None of them wants a button.**
...
> **Re-measured 2026-09-08 (B159):**
> `294 routes · 229 called · 12 external · 40 duplicated · 7 second door · 5 unsurfaced`
>
> The counts in this document are the ones taken when it was written; the GENERATED truth is
> `docs/capability-reconciliation.json`, regenerated by the script. Two changes are worth naming
> rather than leaving to a diff:
>
> * **`external` 8 → 12, `unsurfaced` 8 → 5.** The cron-called sweeps were hand-listed in
>   `EXTERNAL_CALLER` and the list named two of the four, so `space-presence/sweep`,
>   `event-brackets/sweep` and `tasks/sweep-claims` read as dead capability while
>   `pipeline/src/main.py` had been calling all four on a schedule. The list is now DERIVED from
>   middleware's own `CRON_EXACT_PATHS` — one authority, and adding a sweep explains it in the same
>   edit.
> * **The derivation's first version read prose as code.** `'([^']+)'` over the raw file matched
>   apostrophes inside the comments (`middleware's wording`), so a comment fragment parsed as a path
>   and the closing quote swallowed the entry the derivation existed to pick up. Comments are
>   stripped first and every parsed entry is validated as a path, with a throw rather than a silent
>   reclassification. Fourth instrument this cycle to be wrong that exact way.
>
> The dispositions below still stand for the routes that remain — `…/ai/draft` in particular is
> still **dead, not unsurfaced**.
