...
3. **Collaborator library — two-level, derivative of the customer.** A collaborator (`partner_user`) gets a **discrete** library partition under the customer (tenant) who **assigned** them to a proposal. It **persists** for that customer's future proposals but is **locked until re-assigned** (access = an active `proposal_collaborators` assignment). **Discrete per collaborator → no cross-collaborator contamination**; moving an artifact to another collaborator requires **re-upload** (by them, or the customer admin uploading *into that collaborator's* partition). The customer admin may **visualize + ingest** a collaborator's artifacts **only those uploaded + approved-for-atomization for that project**. They get **create-from-template + populate-from-their-library** → they do the work, offloading the customer admin. **Ownership = Option B:** the **customer owns the space** (content is **tenant-partitioned**, so cross-*customer* leak is structurally impossible — a multi-customer collaborator re-uploads per customer), while the **collaborator owns + controls their items** (**restrict / delete**, forward-looking — §9). Each collaborator sets a **posture per customer**: `share_all` (auto-reuse on that customer's future proposals — "shareable until it isn't") or `restrict_until_approved` (names-only until approved); **default = restrict** for safety. This also enables **both directions** (customer pushes templates + assigned sections *down*; collaborator shares content *up*) — vs Option A (collaborator-owned, push-only, a cross-customer leak vector). **RECOMMENDED impl:** `library_units.owner_user_id` (uploader) + `share_default` per (owner, tenant) + per-proposal **`library_unit_shares(unit_id, proposal_id, approved_by, approved_at)`** + per-item restrict flag; the lock is the assignment check, not a column.
...
- **D2 Library scope — RECOMMENDED:** reserved platform tenant (ours; `tenant_id` stays NOT NULL); collaborator = **Option B** (customer owns space, collaborator owns items): `owner_user_id` + `share_default` per (owner,tenant) + per-proposal `library_unit_shares` + per-item restrict + assignment-gated lock (no `tenant_id` change); shared templates = `document_templates.is_system` (held). Recursive draft→approved disclosure; collaborator delete/restrict is forward-looking. **Confirm.**
- **D6 Collaborator ownership — RECOMMENDED Option B** (customer-owned space + collaborator-controlled items, posture default = restrict). **Confirm.**
