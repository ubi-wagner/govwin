...
### COST FAMILY (2026-08) — formula cost sheets that transcribe 1:1 to the portal

Five formula-driven **burden-waterfall cost workbooks** across the common periods of performance —
`cost-phase1-base-option` (6mo + 6mo option), `cost-under-12mo`, `cost-18mo`, `cost-phase2-24mo`
(Year 1 + Year 2), `cost-custom` — generated by one parameterized builder
(`lib/templates/cost/burden-cost-sheet.ts`). Four sheets: **Rates** (the four OH/Fringe/G&A/Fee inputs —
the "easy insert your rates" knobs) · **Labor** (the 9 common program labor codes — PI, Co-I/Senior
Scientist, Senior/Research/Software/Design Engineer, Technician, PM, Business/Admin — × per-period hours ×
unburdened rate) · **ODC** (Materials/Travel/Equipment/Other + Subcontracts per period) · **Summary** (the
burden waterfall per PoP + a Total column). Every roll-up is a live Excel formula.

**The formulas mirror the portal cost engine (`lib/proposal/cost-model.ts` `computeBudget`) exactly**, so a
filled cost template transcribes 1:1 into the portal cost sheet and the numbers agree. Proven by a
formula-evaluation harness that fills inputs, evaluates every cell, and checks against `computeBudget`:
2-period base+option → **$393,689.38** (template) = **$393,689.38** (engine) ✓; single-period → **$300,146.77
= $300,146.77** ✓ (Direct Labor, Fee, and Total Price all match to the cent). The harness caught a real bug
(the price-row label clobbering the first period cell) that was fixed. Program rules are embedded: fee ≤7%
note, SBIR PI ≥50%-hours note, equipment >$5,000 rule, subcontracts pass-through-but-in-G&A-base, ceiling
reminder. (Grants use the existing SF-424A form; the waterfall is agency-universal.)

**Stable now 39 templates.** tsc 0 · vitest 1085.
