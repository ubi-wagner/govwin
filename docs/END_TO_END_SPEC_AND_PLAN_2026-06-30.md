...
**Date:** 2026-06-30 · **Rev:** 2 (final review)
...
3. **Collaborator sub-scope.** Collaborator uploads → **RECOMMENDED:** additive `owner_user_id` + `visibility('tenant'|'owner_only')` on `library_units` (no constraint change; still tenant-partitioned). Collaborators get **create-from-template + populate-from-their-library** → they do the work, offloading the customer admin.
...
- **D2 Library scope — RECOMMENDED:** reserved platform tenant (ours) + `owner_user_id`/`visibility` (collaborator) + `document_templates.is_system` (shared/held). **Confirm.**
...
- **T6.1** Customer library = own + shared templates + scratch; **collaborator** library + create-from-template + populate-from-library + **meta-tagging**; roles: collaborators **mark complete**, admins **lock+accept**. *Test: a collaborator builds + tags a doc from their library; admin locks it.*
...
3. **Collaborator `owner_only` atoms** are visible to that collaborator + tenant admins, never to peer tenants.
