...
- `system_events_namespace_chk` (mig 069, widened by mig 217) — `namespace` ∈ the 8 registry
  values. **This is the only one of the three copies that FAILS rather than warns.** *Proven live:*
...
**8 registry namespaces · 303 distinct literal types** (frontend + pipeline `emit_event`; `[py]` = pipeline). Generated by `python3 scripts/event_catalog.py --write` — do not hand-edit between the markers.
