...
> SPINE_MAP's framing wins; the counts here have been reconciled to it (36 archetypes, ~20 workflows,
> migrations to 137).
...
```
   ...
   pipeline/src/events.py │           shared Postgres, no broker           │
             ...
             namespace != 'system'        ▼               ▼  skip phase=start / error rows
                  ...
                  │  → AgentFabric (36 archetypes)│  │     todo/publish… (§3)        │
...
- `AgentFabric` (`fabric.py`) registers **36 archetypes** (`_ARCHETYPE_CLASSES`,
