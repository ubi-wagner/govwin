...
## In progress (same live rig)
- ai-review (color_team_reviewer), Studio Draft/Refine/Compliance, assess-ingest (rfp_ingest_manager),
  ai/research — event-triggered, same worker path.
- Direct-LLM on `:3001`: atom vision-caption, shred-audit / parse-solicitation, source-scout.
- Semantic retrieval is gated but uses the **local-hash** embedder (`ATOM_EMBED=local`), no LLM call.
