...
# ---------------------------------------------------------------------------
# `000_drop_all.sql` unconditionally drops the public schema. It must
# NEVER run in production unless ALLOW_SCHEMA_RESET=true is set on the
# environment. This matches the behavior of pipeline/src/main.py::run_migrations.
# The guard applies to every code path below: --status, --dry-run,
# the main apply loop, and the checksum computation.
ALLOW_SCHEMA_RESET="${ALLOW_SCHEMA_RESET:-false}"
...
should_skip_destructive() {
  local file="$1"
  if is_destructive_migration "$file" && [[ "$ALLOW_SCHEMA_RESET" != "true" ]]; then
    return 0  # yes, skip it
  fi
