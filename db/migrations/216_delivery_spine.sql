-- 216_delivery_spine.sql
--
-- Post-award delivery management: CLIN · WBS · milestones · deliverables.
--
...
-- Canonical design: docs/DELIVERY_MANAGEMENT_DESIGN.md.
--
...
-- `delivery_milestones.current_date` is in the design doc and **is a syntax error**:
--
...
--   a delivery project is not one; a slumbering tenant darkens through `verifyTenantAccess`.
...
CREATE TABLE IF NOT EXISTS delivery_projects (
  ...
  -- delivery_source_documents below.
...
COMMENT ON COLUMN delivery_projects.contract_id IS
  'Navigation convenience only. The ANCHOR is the uploaded artifacts in delivery_source_documents '
  '— see migration 216 and docs/DELIVERY_MANAGEMENT_DESIGN.md.';

CREATE TABLE IF NOT EXISTS delivery_source_documents (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       uuid NOT NULL REFERENCES tenants(id),
...
COMMENT ON TABLE delivery_source_documents IS
...
-- CLIN → WBS
-- ═════════════════════════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS delivery_clins (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       uuid NOT NULL REFERENCES tenants(id),
  project_id      uuid NOT NULL REFERENCES delivery_projects(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN delivery_clins.funded_amount IS
...
CREATE TABLE IF NOT EXISTS delivery_wbs_nodes (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       uuid NOT NULL REFERENCES tenants(id),
  project_id      uuid NOT NULL REFERENCES delivery_projects(id) ON DELETE CASCADE,
  clin_id         uuid REFERENCES delivery_clins(id) ON DELETE SET NULL,
  parent_id       uuid REFERENCES delivery_wbs_nodes(id) ON DELETE CASCADE,
  code            text NOT NULL,
  title           text NOT NULL,
...
COMMENT ON COLUMN delivery_wbs_nodes.sort_index IS
...
CREATE TABLE IF NOT EXISTS delivery_milestones (
  ...
  project_id      uuid NOT NULL REFERENCES delivery_projects(id) ON DELETE CASCADE,
  clin_id         uuid REFERENCES delivery_clins(id) ON DELETE SET NULL,
  wbs_node_id     uuid REFERENCES delivery_wbs_nodes(id) ON DELETE SET NULL,
  title           text NOT NULL,
  baseline_date   date,
...
COMMENT ON COLUMN delivery_milestones.forecast_date IS
...
COMMENT ON TABLE delivery_deliverables IS
...
-- ═════════════════════════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS delivery_provenance (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       uuid NOT NULL REFERENCES tenants(id),
  project_id      uuid NOT NULL REFERENCES delivery_projects(id) ON DELETE CASCADE,
  ...
  source_doc_id   uuid REFERENCES delivery_source_documents(id) ON DELETE SET NULL,
  page            integer,
  excerpt         text,
  char_offset     integer,
  created_by      uuid REFERENCES users(id),
...
COMMENT ON TABLE delivery_provenance IS
...
-- ═════════════════════════════════════════════════════════════════════════════════════════════
CREATE INDEX IF NOT EXISTS idx_delivery_projects_tenant       ON delivery_projects (tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_delivery_projects_contract     ON delivery_projects (contract_id) WHERE contract_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_delivery_source_docs_project   ON delivery_source_documents (project_id);
CREATE INDEX IF NOT EXISTS idx_delivery_clins_project         ON delivery_clins (project_id, sort_index);
CREATE INDEX IF NOT EXISTS idx_delivery_wbs_project           ON delivery_wbs_nodes (project_id, sort_index);
CREATE INDEX IF NOT EXISTS idx_delivery_wbs_parent            ON delivery_wbs_nodes (parent_id) WHERE parent_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_delivery_wbs_clin              ON delivery_wbs_nodes (clin_id) WHERE clin_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_delivery_milestones_project    ON delivery_milestones (project_id, sort_index);
CREATE INDEX IF NOT EXISTS idx_delivery_milestones_clin       ON delivery_milestones (clin_id) WHERE clin_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_delivery_milestones_due        ON delivery_milestones (forecast_date) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_delivery_deliverables_ms       ON delivery_deliverables (milestone_id, sort_index);
CREATE INDEX IF NOT EXISTS idx_delivery_provenance_target     ON delivery_provenance (target_table, target_id);
CREATE INDEX IF NOT EXISTS idx_delivery_assignments_user      ON delivery_assignments (user_id);
CREATE INDEX IF NOT EXISTS idx_delivery_assignments_project   ON delivery_assignments (project_id);
...
-- `delivery_deliverables` has no `project_id`; its tenancy is its own `tenant_id` column, which the
-- policy above already covers. The FK to `delivery_milestones` is navigational, not the tenancy
...
-- ═════════════════════════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION delivery_baseline_is_immutable() RETURNS trigger AS $$
DECLARE
  col text;
  old_val text;
  new_val text;
BEGIN
  FOREACH col IN ARRAY TG_ARGV LOOP
    ...
    IF old_val IS NOT NULL AND old_val IS DISTINCT FROM new_val THEN
      RAISE EXCEPTION
        'delivery: % is a BASELINE column and is written once (% -> %). Rebaseline supersedes the '
...
DROP TRIGGER IF EXISTS trg_wbs_baseline_immutable ON delivery_wbs_nodes;
CREATE TRIGGER trg_wbs_baseline_immutable
  BEFORE UPDATE ON delivery_wbs_nodes
  FOR EACH ROW
  EXECUTE FUNCTION delivery_baseline_is_immutable('baseline_start', 'baseline_end', 'baseline_cost');

DROP TRIGGER IF EXISTS trg_milestone_baseline_immutable ON delivery_milestones;
CREATE TRIGGER trg_milestone_baseline_immutable
  BEFORE UPDATE ON delivery_milestones
  FOR EACH ROW
  EXECUTE FUNCTION delivery_baseline_is_immutable('baseline_date');

DROP TRIGGER IF EXISTS trg_project_baselined_immutable ON delivery_projects;
CREATE TRIGGER trg_project_baselined_immutable
  BEFORE UPDATE ON delivery_projects
  FOR EACH ROW
  EXECUTE FUNCTION delivery_baseline_is_immutable('baselined_at');
